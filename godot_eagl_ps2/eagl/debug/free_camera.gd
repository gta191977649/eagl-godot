...
var _wants_mouse_capture := false
...
func _update_speed_hud() -> void:
	if speed_label == null:
		return
	mouse_captured = _is_mouse_captured()
	var mode := "captured" if mouse_captured else "click scene to aim"
	speed_label.text = "Free camera\nSpeed: %.1f  Sprint: %.1f\nWheel / +/- / [] adjust, 0 reset\nWASD move, Space/E up, Ctrl/Q down, Shift fast, Alt slow, Esc: %s" % [
		base_speed,
