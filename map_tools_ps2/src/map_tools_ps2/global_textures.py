"""Batch extraction of PS2 GLOBAL texture libraries."""
...
def _resource_candidates(global_dir: Path) -> list[Path]:
    paths = [path for path in global_dir.rglob("*") if path.is_file() and path.suffix.upper() in SUPPORTED_SUFFIXES]
    paths.sort(key=lambda path: (str(path.relative_to(global_dir)).lower(), path.suffix.upper() != ".BUN"))

    # A compressed LZC and its decompressed BUN are the same resource.
    by_stem: dict[Path, Path] = {}
    for path in paths:
        key = path.with_suffix("").relative_to(global_dir)
        previous = by_stem.get(key)
        if previous is None or (path.suffix.upper() == ".BUN" and previous.suffix.upper() == ".LZC"):
            by_stem[key] = path
    return sorted(by_stem.values(), key=lambda path: str(path.relative_to(global_dir)).lower())


def export_global_textures(global_dir: Path, output_dir: Path) -> dict[str, object]:
    ...
    resources = _resource_candidates(global_dir)
    manifest: dict[str, object] = {
        "global_dir": str(global_dir),
        "output_dir": str(output_dir),
        "resources": [],
        "unprocessed_files": [
            {"source": str(path.relative_to(global_dir)), "reason": "unsupported extension"}
            for path in sorted(global_dir.rglob("*"))
    ...
    report_progress("Scanning GLOBAL texture resources", 0, len(resources), None)
    for index, source_path in enumerate(resources, 1):
        relative = source_path.relative_to(global_dir)
        ...
        report_progress("Extracting GLOBAL textures", index, len(resources), str(relative))

    manifest["resources"] = records
    manifest_path = output_dir / "global_manifest.json"
