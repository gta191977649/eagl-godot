...
def _report(stage: str, current: int, total: int) -> None:
    """Stream progress to the parent process, which owns the terminal."""
    print(f"MTA_PROGRESS {stage}	{current}	{total}", flush=True)
...
def main() -> None:
    ...
    for model_index, model in enumerate(manifest["models"], 1):
        _report("Writing DFF/COL", model_index, total_models)
    ...
    _report("Writing TXD", 0, len(manifest["textures"]))
    _export_txd(txd_module, manifest, images, txd_path)
    _report("Writing TXD", len(manifest["textures"]), len(manifest["textures"]))
    max_dff_vertices = 0
    prelit_geometries = 0
